...
impl Camera{
    ...
    pub fn update(&mut self,player_pos: [f32;2],input: &InputManager,egpu: &mut easy_gpu::Renderer){
        const FOLLOW_SPEED: f32 = 3.;

        let diff = [player_pos[0] - self.data.position[0],player_pos[1] - self.data.position[1]];
        self.data.position = [self.data.position[0] + (diff[0]/FOLLOW_SPEED),self.data.position[1] + (diff[1]/FOLLOW_SPEED)];
